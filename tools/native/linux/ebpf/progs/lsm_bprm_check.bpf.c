...
/*
 * lsm_bprm_check.bpf.c
 *
 * BPF LSM program attached at lsm/bprm_check_security to gate exec().
 * The BPF helper `bpf_d_path` is used (when available) to fill the
 * absolute path of the binary into the event so userspace can match
 * policy. Falls back to comm if bpf_d_path is unavailable.
 */
...
int BPF_PROG(tf_bprm_check, struct linux_binprm *bprm, int ret)
{
	if (ret != 0) return ret;
	if (!bprm)    return 0;
